...
//   MESMAS variáveis CSS do céu (--v3-sky-top/mid/low). O vidro reflete a
...
function readSkyColors(root) {
    ...
    return {
        top: read('--v3-sky-top', '#cfe4f7'),
        mid: read('--v3-sky-mid', '#e7f1fb'),
        low: read('--v3-sky-low', '#fbfdff'),
        ink: read('--v3-ink', '#25303f'),
    };
}
 ...
 *
 * @param {HTMLElement} mount   contêiner com [data-v3-3d]
...
export async function createSky3d(mount, { layout, marks }) {
    ...
    for (let index = 0; index < used.length; index += 1) {
        ...
        // A cor vem do CUBO que a própria página desenhou (data-v3-color no
        // Blade): uma fonte de verdade para o fallback e para o 3D.
        const color = svg.closest('[data-v3-color]')?.dataset.v3Color ?? '#6470b0';
        ...
        // Logo em decal BRANCO no meio da face.
        const canvas = await markToCanvas(svg, 256, '#ffffff');
        const texture = new THREE.CanvasTexture(canvas);
        texture.colorSpace = THREE.SRGBColorSpace;
    ...
    document.addEventListener('v3:scene', onScene);
    ...
    mount.dataset.v3ThreeReady = 'true';
    ...
    return function destroy() {
        ...
        document.removeEventListener('v3:scene', onScene);
        ...
        delete mount.dataset.v3ThreeReady;
    };
}

// Motion da landing — UMA cena, dirigida pelo scroll.
//
...
function initCounters(gsap, ScrollTrigger, reducedMotion) {
    const nodes = [...document.querySelectorAll('[data-sky-count]')];
    const format = new Intl.NumberFormat(document.documentElement.lang || undefined);

    for (const node of nodes) {
        const target = Number(node.dataset.skyCount);
...
async function initTextReveals(gsap, ScrollTrigger, SplitText) {
    ...
    for (const element of document.querySelectorAll('[data-sky-text]')) {
        ...
        const split = new SplitText(element, { type: 'lines', mask: 'lines', linesClass: 'sky-line' });
...
function initPointerParallax(hero, reducedMotion) {
    ...
    const frame = () => {
        x += (targetX - x) * 0.06;
        y += (targetY - y) * 0.06;
        hero.style.setProperty('--sky-px', x.toFixed(4));
        hero.style.setProperty('--sky-py', y.toFixed(4));
...
export function initMotion({ gsap, ScrollTrigger, SplitText, Lenis, reducedMotion }) {
    const revealed = [...document.querySelectorAll('.sky-rise')];
    ...
    if (hero !== null) {
        [...hero.querySelectorAll('.sky-rise')].forEach((element, index) => {
            element.style.setProperty('--sky-delay', `${index * 90}ms`);
    ...
    if (hero !== null) {
        ...
        const fan = hero.querySelector('[data-sky-fan]');
        const cards = fan === null ? [] : [...fan.querySelectorAll('[data-sky-fan-card]')];
        ...
        timeline
            .to('[data-sky-layer="back"]', { yPercent: 10, scale: 1.1, ease: 'none' }, 0)
            .to('[data-sky-layer="mid"]', { yPercent: -7, ease: 'none' }, 0)
            .to('[data-sky-layer="front"]', { yPercent: -16, opacity: 0.12, ease: 'none' }, 0)
            .to(progress, {
                value: 1,
                ease: 'none',
                onUpdate: () => {
                    ...
                    document.dispatchEvent(new CustomEvent('sky:scene', { detail: { progress: progress.value } }));
                },
            }, 0);

...
{{-- Select com label e estado de erro. Opções via slot. Mesma linguagem
     (superfície, borda, altura, foco) do <x-input>. --}}
@php
...
<div {{ $attributes->only('class') }}>
    ...
    <select
        ...
        {{ $attributes->except('class')->merge(['class' =>
            'block w-full rounded-lg border bg-surface px-3 py-2.5 text-sm text-gray-900 transition-[border-color,background-color] duration-150 ease-(--ease-out) focus:outline-none focus:ring-2 focus:ring-brand/50 disabled:cursor-not-allowed disabled:bg-surface-disabled disabled:text-text-muted sm:py-2 dark:text-gray-100 '
            .($hasError

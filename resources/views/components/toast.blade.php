...
<div role="status" {{ $attributes->merge(['class' => 'flex items-center gap-3 rounded-lg border bg-white p-4 text-sm shadow-lg dark:bg-gray-900 '.$styles[0]]) }}>
    <x-ui-icon :name="$styles[1]" class="h-5 w-5 shrink-0" />
    <div class="text-gray-700 dark:text-gray-200">{{ $slot }}</div>
</div>

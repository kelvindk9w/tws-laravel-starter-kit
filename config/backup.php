...
return [
    ...
    'backup' => [
         ...
         */
        // O dump é comprimido (gzip) antes de entrar no zip criptografado.
        ...
        'destination' => [
             ...
             */
            // Discos de destino (separados por vírgula no .env). Em dev:
            // 'local' (storage/app/private/<APP_NAME>). Em produção:
            // 'backup' (Cloudflare R2 — ver config/filesystems.php).
         ...
         * `?: null`: variável presente e vazia (.env.example) = sem senha.
         */
         ...
         */
        // Reabre o zip recém-criado e confere o conteúdo (overhead mínimo
        // em dumps só de banco; vale a prova extra de integridade).
    ...
    'notifications' => [
        ...
        'notifications' => [
            BackupHasFailedNotification::class => ['mail', 'webhook'],
            UnhealthyBackupWasFoundNotification::class => ['mail', 'webhook'],
            CleanupHasFailedNotification::class => ['mail', 'webhook'],
            BackupWasSuccessfulNotification::class => ['webhook'],
    ...
    'monitor_backups' => [
        [
            'name' => env('APP_NAME', 'laravel-backup'),
            // Mesmos discos do destino — o monitor vigia onde o dump grava.
            'disks' => array_filter(array_map('trim', explode(',', (string) env('BACKUP_DISKS', 'local')))),
            'health_checks' => [
                // Idade máxima do backup mais recente (dias) e teto de
                // armazenamento (MB) — verificados pelo backup:monitor diário.
                MaximumAgeInDays::class => (int) env('BACKUP_MONITOR_MAX_AGE_DAYS', 1),
                MaximumStorageInMegabytes::class => (int) env('BACKUP_MONITOR_MAX_STORAGE_MB', 5000),
            ],
        ],

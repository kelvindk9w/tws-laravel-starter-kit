...
`APP_KEY`, como se a variável não existisse. A regra está no
`config/api_keys.php` e, de novo, no `App\Core\ApiKeys\Support\ApiKeyHasher` —
o único ponto que entrega o pepper ao HMAC, incluindo o hash fictício da pk_
inexistente —, então vale mesmo quando o `config/api_keys.php` da instalação
ficou na versão antiga (por exemplo, customizado e não atualizado). Sem pepper dedicado e

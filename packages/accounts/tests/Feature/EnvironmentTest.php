...
it('não herda nenhuma variável do kit do ambiente, só as fixadas no phpunit.xml', function (): void {
    $doKit = array_values(array_filter(
        array_keys(getenv()),
        fn (string $name): bool => preg_match('/^(API_KEYS_|RATE_LIMIT_|AUTH_|SECURITY_|PLATFORM_|REQUEST_LOG_|AUDIT_|BACKUP_|LOG_|REDIS_|DB_(?!CONNECTION$)|SESSION_(?!DRIVER$)|MAIL_(?!MAILER$))/', $name) === 1,
    ));

    expect($doKit)->toBe([])
        ->and(env('APP_LOCALE'))->toBe('en')
        ->and(app()->getLocale())->toBe('en')
        ->and(env('API_KEYS_HASH_PEPPER'))->toBeNull()
        ->and(config('api_keys.inactivity.months'))->toBe(3);
});

// E2E da landing "Céu" (v3) — o que só o navegador prova: o split-screen
// arrastável, o bento vivo, a cena pinada do herói, o fallback do 3D e a
// ausência de scroll lateral no celular.
//
// Rodar: npx playwright test tests/e2e/landing-v3.spec.js
import { test, expect } from '@playwright/test';

const DESKTOP = { width: 1440, height: 900 };
const MOBILE = { width: 390, height: 844 };

test.describe('landing v3 — /v3', () => {
    test('as cinco seções chegam ao navegador', async ({ page }) => {
        ...
        await expect(page.locator('#seguranca')).toBeAttached();
        await expect(page.locator('[data-v3-3d-layout="arc"], [data-v3-layout="arc"]')).toBeAttached();
    });
    ...
    test('o leque mostra as quatro telas reais, já carregadas', async ({ page }) => {
        ...
        const cards = page.locator('[data-v3-fan-card]');
    ...
    test('o split-screen é arrastável no ponteiro e no teclado', async ({ page }) => {
        ...
        const split = page.locator('[data-v3-split]');
        await split.scrollIntoViewIfNeeded();

        const range = split.locator('[data-v3-split-range]');
        ...
        await range.focus();
        const before = await split.evaluate((el) => el.style.getPropertyValue('--v3-split'));
        await range.press('ArrowRight');
        await range.press('ArrowRight');
        await range.press('ArrowRight');
        const after = await split.evaluate((el) => el.style.getPropertyValue('--v3-split'));
        ...
        const dragged = Number.parseFloat(await split.evaluate((el) => el.style.getPropertyValue('--v3-split')));
    ...
    test('o bento acende uma célula por vez e para no ponteiro', async ({ page }) => {
        ...
        const bento = page.locator('[data-v3-bento]');
        await bento.scrollIntoViewIfNeeded();

        await expect(bento.locator('[data-v3-cell]')).toHaveCount(6);
    ...
    test('a cena do herói é dirigida pelo scroll e depois solta a página', async ({ page }) => {
        ...
        const hero = page.locator('[data-v3-hero]');
    ...
    test('sem WebGL a página continua inteira: o fallback de vidro fica', async ({ page }) => {
        ...
        await page.addInitScript(() => {
            // Simula um aparelho sem WebGL2 (o guard da v3 desiste antes de
            // baixar o Three.js).
        ...
        await expect(page.locator('[data-v3-three-ready]')).toHaveCount(0);
        await expect(page.locator('[data-v3-3d-fallback] .v3-tech-cube').first()).toBeVisible();
